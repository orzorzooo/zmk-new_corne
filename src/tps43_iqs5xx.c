...
static void iqs5xx_work_handler(struct k_work *work) {
    ...
    if (hold_became_active) {
        ...
        /* Keep both halves of a tap in the same trackpad event. A delayed release on
         * the peripheral can be dropped independently by the split transport and
         * leave the host with a permanently held mouse button. */
