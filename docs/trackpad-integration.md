...
| 方向設定 | 僅 `switch-xy`（兩個 flip 都不加）— 依實測推導，**待刷機驗證** |
...
| 設定 | 實測結果 |
|---|---|
| `switch-xy` + `flip-x` | 上下正確，**左右相反** |
| `switch-xy`（最終採用） | 左右相反是 `flip-x` 翻轉 X 輸出造成的，拿掉即正確 |

註：專案最初就是 `switch-xy`，後來誤判為「需要順時針轉 90 度」而加了 `flip-x`。當時的誤判可能來自韌體沒刷到右半（設定從未寫進晶片）或觸控板後來重新安裝過。

## 異動紀錄（trackpad 分支）

| Commit | 內容 |
|---|---|
| （未 commit）2026-07-16 | 方向定案為僅 `switch-xy`：實測 `switch-xy + flip-x` 為「上下正確、左右相反」，依 datasheet flip 作用於最終輸出軸，移除 `flip-x` 修正左右（期間曾短暫改為 `flip-y`，未刷機即被此結論取代） |
